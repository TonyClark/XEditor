...
public class ForeignFuns implements Value, Instr, Errors {
	...
	public static void Kernel_fork(Machine machine) {
		...
		if (machine.funArity(fun) == 0) {
			...
			error(ARGCOUNT, machine, "Fork expects a 0-arity function.", fun, Machine.nilValue);
	}
	...
	public static void Kernel_invokeMethod(Machine machine) {
		...
		if (object instanceof java.lang.reflect.Method && arity == 2) {
			...
			machine.error(ERROR, "Cannot invoke foreign object.");
	}
	...
	public static void Kernel_load(Machine machine) {
		try {
			...
			if (Machine.isString(source))
				...
				error(TYPE, machine, "Kernel_load: expecting a filename or an input channel: " + machine.valueToString(source));
			machine.popFrame();
	...
	public static void Kernel_readXML(Machine machine) {
		...
		if (Machine.isString(source))
			...
			error(TYPE, machine, "Kernel_realXML expects a string or an input channel: " + machine.valueToString(source));
		machine.popFrame();
	}
	...
	public static void Kernel_sendForeignInstance(Machine machine) {
		...
		if (!Machine.isSymbol(message))
			message = machine.mkSymbol(message);
		int args = machine.frameLocal(2);
		machine.popFrame();
		machine.popStack();
	...
	public static void Kernel_setSlotVisibility(Machine machine) {
		...
		machine.objSetAttVisibility(object, name, visibility);
		machine.popFrame();
	}
	...
	public static void Kernel_symbolSetName(Machine machine) {
		...
		if (Machine.isSymbol(symbol) && Machine.isString(name)) {
			...
			error(TYPE, machine, "Kernel_symbolSetName expects a symbol and a string: " + machine.valueToString(symbol) + " " + machine.valueToString(name));
	}
	...
	public static void Kernel_toJava(Machine machine) {
		...
		if (Machine.isForeignObj(type)) {
			...
			if (object instanceof java.lang.Class<?>) {
				...
				error(TYPE, machine, "expecting a Java class in toJava: " + machine.valueToString(type));
		}
	}
