...
public final class OperatingSystem implements EventHandler {
	...
	private Vector<InputStream> inputChannels = new Vector<InputStream>();
	...
	private Vector<StreamMonitor> inputMonitors = new Vector<StreamMonitor>();
	...
	private Vector<String> internalClientNames = new Vector<String>();

	// XOS is supplied with the names of message clients via the -message
	// command line argument. The value of the argument should be the
	// qualified name of the class relative to CLASSPATH.

	private Vector<String> messageClientNames = new Vector<String>();

	// Message monitors are used to detect incoming messages for message
	// based clients.

	private Vector<MessageMonitor> messageMonitors = new Vector<MessageMonitor>();

	// A pool of free messages that can be used by clients and XVM.

	private Stack<Message> messagePool = new Stack<Message>();

	// A pool of free packages for messages that can be used by clients and XVM.

	private Stack<MessagePacket> packetPool = new Stack<MessagePacket>();
	...
	private Vector<OutputStream> outputChannels = new Vector<OutputStream>();
	...
	// XOS may be started via Eclipse in which case we expect different moding
	// of things like stdin and stdout...
	...
	private Stack<Value> valuePool = new Stack<Value>();
	...
	private Vector<String> waitingMessageClients = new Vector<String>();
	...
	private FileResolver fileResolver = new BasicFileResolver();
	...
	public void initConnectionMonitor(int port) {
		...
		// If no port has been specified then do not start the monitor. The
		// initial
		// value for port is -1 and will not have changed if no port is
		// specified.
	...
	public void initInternalClient(String className, String clientName) {
		try {
			Class<?> clientClass = Class.forName(className);
			Constructor<?> constructor = clientClass.getDeclaredConstructor(new Class<?>[] { InputStream.class, OutputStream.class });
	...
	public synchronized void initMessageClients() {
		...
		// constructor. To initialise the client it must implement
		// MessageHandler.
	...
	public int newURLInputChannel(String urlString) {
		...
		Authenticator.setDefault(new Authenticator() {
			...
			protected PasswordAuthentication getPasswordAuthentication() {
				...
				// documented that can
				// be null
				// String scheme = getRequestingScheme(); // not documented that
				// can be null
	...
	public synchronized void removeWaitingMessageClient(String name) {
		...
		// XOS and XOS may be waiting for the client to connect before starting
		// up.
	...
	public synchronized void scheduleClientThreads(String name) {
		...
		Vector<Thread> threads = (Vector<Thread>) clientThreads.clone();
	...
	public Thread getScheduledThread(int thread) {
		for (Thread t : scheduledThreads) {
			if (t.id() == thread)
				return t;
		}
		return null;
