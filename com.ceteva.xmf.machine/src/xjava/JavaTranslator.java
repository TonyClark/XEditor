...
public class JavaTranslator {
  ...
  public static int mapJavaValue(Machine machine, Object value) {
    if (value == null) return Machine.undefinedValue;
    if (value instanceof Boolean) return ((Boolean) value).booleanValue() ? Machine.trueValue : Machine.falseValue;
    if (value instanceof Integer) return Machine.mkInt(((Integer) value).intValue());
    if (value instanceof Float) return machine.mkFloat(((Float) value).floatValue());
    if (value instanceof Double) return machine.mkFloat(((Double) value).floatValue());
    if (value instanceof String) return machine.mkString((String) value);
    if (value instanceof Object[]) return mapJavaArray(machine, (Object[]) value);
    if (value instanceof int[]) return mapJavaArray(machine, (int[]) value);
    if (value instanceof boolean[]) return mapJavaArray(machine, (boolean[]) value);
    if (value instanceof float[]) return mapJavaArray(machine, (float[]) value);
    if (value instanceof double[]) return mapJavaArray(machine, (double[]) value);
    if (value instanceof Vector<?>) return mapJavaVector(machine, (Vector<?>) value);
    if (value instanceof Hashtable) return mapJavaHashtable(machine, (Hashtable<?, ?>) value);
  ...
  public static Object mapXMFConsToArray(Machine machine, Class<?> type, int value) {
    if (type == Integer.TYPE) return mapXMFConsToIntArray(machine, value);
    if (type == Boolean.TYPE) return mapXMFConsToBoolArray(machine, value);
    /*
  ...
  public static int[] mapXMFConsToIntArray(Machine machine, int values) {
    ...
    while (values != Machine.nilValue) {
      ...
      } else throw new XMFToJavaTypeError("Expecting an int", value, Integer.TYPE);
    }
    return array;
  }
  ...
  public static boolean[] mapXMFConsToBoolArray(Machine machine, int values) {
    ...
    while (values != Machine.nilValue) {
      ...
      if (Machine.isBool(value)) {
        ...
        values = machine.consTail(values);
      } else throw new XMFToJavaTypeError("Expecting a boolean", value, Boolean.TYPE);
    }
    return array;
  }

  public static Object[] mapXMFConsToObjectArray(Machine machine, Class<?> type, int values) {
    Object[] array = (Object[]) Array.newInstance(type, machine.consLength(values));
    int i = 0;
    while (values != Machine.nilValue) {
      int value = machine.consHead(values);
  ...
  private static Object mapXMFTableToHashtable(Machine machine, int table) {
    int length = machine.arrayLength(table);
    Hashtable<Object, Object> javaTable = new Hashtable<Object, Object>();
    for (int i = 0; i < length; i++) {
      ...
      while (bucket != Machine.nilValue) {
        ...
        Object javaKey = mapXMFValue(machine, java.lang.Object.class, key);
        Object javaValue = mapXMFValue(machine, java.lang.Object.class, value);
        javaTable.put(javaKey, javaValue);
      }
    }
    return javaTable;
  }
  ...
  public static Object mapXMFValue(Machine machine, Class<?> type, int value) {
    ...
    if (Machine.isTable(value)) {
      if (type == java.util.Hashtable.class || type == java.lang.Object.class)
        return mapXMFTableToHashtable(machine, value);
      else throw new XMFToJavaTypeError("Table cannot be converted", value, type);
    }
    if (Machine.isNil(value)) {
      if (type.isArray())
        ...
        return new Vector<Object>();
      else throw new XMFToJavaTypeError("Seq{} cannot be converted", value, type);
    }
    return null;
  }
