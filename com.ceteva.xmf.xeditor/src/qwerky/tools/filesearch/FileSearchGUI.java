 ...
 */
package qwerky.tools.filesearch;
...
public class FileSearchGUI implements ActionListener, SearchListener {
	...
	public FileSearchGUI(console.Console console) {
		this(console, null);
	}

	public FileSearchGUI(console.Console console, File dir) {
		frame = new JFrame("File Search");
		this.console = console;
		try {
			Point p = MouseInfo.getPointerInfo().getLocation();
			frame.setBounds((int) p.getX(), (int) p.getY(), 500, 600);
		} catch (Exception ex) {
	...
	private void createWidgets() {
		...
		controlPanel.setPreferredSize(new Dimension(490, 250));

		JLabel dirLabel = new JLabel("Directory to search");
		controlPanel.add(dirLabel);
		searchDirField = new JTextField(35);
		searchDirField.addActionListener(this);
		searchDirField.setBorder(BorderFactory.createLoweredBevelBorder());
		...
		fileNameField.setBorder(BorderFactory.createLoweredBevelBorder());
		controlPanel.add(fileNameField);

		JLabel searchLabel = new JLabel("Search term pattern");
		controlPanel.add(searchLabel);
		searchTermField = new JTextField(35);
		searchTermField.addActionListener(this);
		searchTermField.setBorder(BorderFactory.createLoweredBevelBorder());
	...
	public void setSearchDir(File searchDir) {
		Path p = Paths.get(searchDir.toURI());
		searchDirField.setText(p.normalize().toString());
	}
	...
	private void edit(SearchResponse result) {
		console.edit(result);
	}
