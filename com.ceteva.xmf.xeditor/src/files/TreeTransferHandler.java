	...
	public boolean importData(TransferHandler.TransferSupport support) {
		...
		for (int i = 0; i < nodes.length; i++) {
			if (JOptionPane.showConfirmDialog(null, "Confirm drop of " + nodes[i].toString(), "Confirm Drop", JOptionPane.OK_CANCEL_OPTION) == JOptionPane.OK_OPTION) {
				if (nodes[i] instanceof FileSystemNode && parent instanceof DirectoryNode) {
					DirectoryNode dirNode = (DirectoryNode) parent;
					FileSystemNode fileSystemNode = (FileSystemNode) nodes[i];
					FileSystemNode newNode = fileSystemNode.copyInto(dirNode,remove);
					if (newNode != null) {
