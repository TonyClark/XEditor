...
public class FilmstripTab extends DiagramTab {
	...
	private JSplitPane    leftSplit   = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT);
	private JSplitPane    treeSplit   = new JSplitPane(JSplitPane.VERTICAL_SPLIT);
	...
	public FilmstripTab(Console console, DiagramFrame frame, String name, Filmstrip filmstrip) {
		...
		populateStepTree();
		stepTree.expandAllNodes();
		checkPanel = new CheckPanel(filmstrip);
		treeSplit.setTopComponent(new JScrollPane(stepTree));
		treeSplit.setBottomComponent(new JScrollPane(checkPanel));
		leftSplit.setLeftComponent(treeSplit);
		leftSplit.setRightComponent(currentStep);
